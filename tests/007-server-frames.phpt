--TEST--
WebSocket\Server handles text, binary, ping, and close frames
--EXTENSIONS--
websocket
--SKIPIF--
<?php
...
$tmpDir = sys_get_temp_dir() . '/websocket-server-frames-test-' . getmypid();
...
$server->onOpen(static function (Connection $connection): void {
    ...
    $connection->send('welcome', MessageType::Text);
});
...
if ($client !== false) {
    ...
    $welcomeBuffer = $headerEnd === false ? '' : substr($responseBuffer, $headerEnd + 4);
    $welcome = $readFrame($client, $welcomeBuffer);

    fwrite($client, Protocol::pack('hello', Protocol::OPCODE_TEXT, Protocol::FLAG_FIN | Protocol::FLAG_MASK));
    $textEcho = $readFrame($client);

    fwrite($client, Protocol::pack("\x00\x01", Protocol::OPCODE_BINARY, Protocol::FLAG_FIN | Protocol::FLAG_MASK));
    $binaryEcho = $readFrame($client);

    fwrite($client, Protocol::pack('pi', Protocol::OPCODE_PING, Protocol::FLAG_FIN | Protocol::FLAG_MASK));
    $pong = $readFrame($client);

    fwrite($client, Protocol::pack(pack('n', Protocol::CLOSE_NORMAL) . 'bye', Protocol::OPCODE_CLOSE, Protocol::FLAG_FIN | Protocol::FLAG_MASK));
...
var_dump($welcome instanceof Frame && $welcome->type === MessageType::Text && $welcome->payload === 'welcome');
var_dump($textEcho instanceof Frame && $textEcho->type === MessageType::Text && $textEcho->payload === 'echo:hello');
var_dump($binaryEcho instanceof Frame && $binaryEcho->type === MessageType::Binary && $binaryEcho->payload === "\x00\x01");
var_dump($pong instanceof Frame && $pong->type === MessageType::Pong && $pong->payload === 'pi');
var_dump($close instanceof CloseFrame && $close->code === Protocol::CLOSE_NORMAL && $close->reason === 'bye');
var_dump($events);
var_dump($stdout === '');
var_dump($stderr === '');
...
array(5) {
  ...
  string(4) "open"
  [1]=>
  string(15) "Text:68656c6c6f"
  [2]=>
  string(11) "Binary:0001"
  [3]=>
  string(5) "close"
  [4]=>
  string(8) "returned"
